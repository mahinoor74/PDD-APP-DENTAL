...
export class ExcelTestReporter {
  constructor(options = {}) {
    this.outputFileName = options.outputFileName || 'UI_UX_Design_Test_Execution_Report.xlsx';
    this.outputDir = options.outputDir || path.resolve(process.cwd(), 'reports');
    this.reportTitle = options.reportTitle || 'Detailed UI/UX Design & Micro-interaction Test Execution Logs';
    this.testResults = [];
  }
  ...
  addResult(result) {
    this.testResults.push({
      ...
      platform: result.platform || 'Cross-Browser / Mobile UI',
      testtype: result.testtype || 'UI/UX & Design Audit',
      ...
      outputLogs: result.outputLogs || '[INFO] UI/UX audit point passed.'
    });
  }
  ...
  async generateReport() {
    ...
    workbook.creator = 'ToothMate UI/UX Design QA Automation Engine';
    workbook.created = new Date();

    const logSheet = workbook.addWorksheet('Detailed UI/UX Logs', { views: [{ showGridLines: true }] });
