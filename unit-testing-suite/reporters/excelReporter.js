...
export class ExcelTestReporter {
  constructor(options = {}) {
    this.outputFileName = options.outputFileName || 'Unit_Component_Test_Execution_Report.xlsx';
    this.outputDir = options.outputDir || path.resolve(process.cwd(), 'reports');
    this.reportTitle = options.reportTitle || 'Detailed Unit & Component Test Execution Logs';
    this.testResults = [];
  }
  ...
  addResult(result) {
    this.testResults.push({
      ...
      platform: result.platform || 'Node.js / Vitest Unit',
      testtype: result.testtype || 'Unit & Component Test',
      ...
      outputLogs: result.outputLogs || '[INFO] Unit assertion passed.'
    });
  }
  ...
  async generateReport() {
    ...
    workbook.creator = 'ToothMate Unit QA Automation Engine';
    workbook.created = new Date();

    const logSheet = workbook.addWorksheet('Detailed Unit Logs', { views: [{ showGridLines: true }] });
