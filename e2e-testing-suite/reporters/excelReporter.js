...
/**
 * Custom Excel Reporter for Selenium Web & Appium Mobile E2E Test Execution Logs
 * Matches the exact layout, header styling, status colors, and schema of the target report.
 */
export class ExcelTestReporter {
  ...
  async generateReport() {
    ...
    workbook.creator = 'ToothMate Automated E2E QA Test Engine';
    workbook.lastModifiedBy = 'Selenium-Appium Node.js Suite';
    workbook.created = new Date();

    // ─────────────────────────────────────────────────────────────
    // SHEET 1: Detailed Web Selenium E2E Test Execution Logs
    // ─────────────────────────────────────────────────────────────
    const logSheet = workbook.addWorksheet('Detailed E2E Logs', {
      views: [{ showGridLines: true }]
    });

    // Main Title Header Row
    logSheet.mergeCells('A1:H1');
    const titleCell = logSheet.getCell('A1');
    titleCell.value = 'Detailed Web Selenium E2E Test Execution Logs';
    titleCell.font = { name: 'Segoe UI', size: 14, bold: true, color: { argb: 'FF000000' } };
    titleCell.alignment = { vertical: 'middle', horizontal: 'left' };
    logSheet.getRow(1).height = 30;

    // Table Column Header Row (Row 3)
    const headers = [
      'testcaseid',
      'platform',
      'testtype',
    ...
    this.testResults.forEach((test) => {
      ...
      // Status Column Special Formatting (Green for PASS, Red for FAIL)
      ...
      if (test.status === 'PASS') {
        ...
        statusCell.fill = {
          type: 'pattern',
          pattern: 'solid',
          fgColor: { argb: 'FFFCE4D6' } // Light red/pink fill
        };
    ...
    // ─────────────────────────────────────────────────────────────
    // SHEET 2: Executive Analytics & Pass Rate KPI Dashboard
    // ─────────────────────────────────────────────────────────────
    ...
    summaryTitle.value = 'ToothMate E2E Test Execution Executive Dashboard';
    ...
    summarySheet.getCell('A3').value = 'KPI Metrics Overview';
    ...
    const kpiData = [
      ...
      ['Pass Rate Percentage', `${passPercentage}%`],
      ['Average Execution Time (s)', `${avgDuration}s`]
    ];
    ...
    console.log(`\n📊 EXCEL E2E REPORT GENERATED SUCCESSFULLY AT:\n   ${reportFilePath}\n`);
