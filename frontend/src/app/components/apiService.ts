...
export const DETECTED_PC_IP = "172.19.187.56";
...
export const getApiBaseUrl = (): string => {
  if (typeof window !== "undefined") {
    ...
    // 1. Web & Mobile Browser (Uses Vite Dev Proxy on same port 5173 - Zero Firewall Block!)
    if (protocol === "http:" || protocol === "https:") {
      const savedIp = localStorage.getItem("custom_backend_ip");
      if (savedIp && savedIp.includes(":8000")) {
        return `http://${savedIp.replace(/^https?:\/\//, "").replace(/\/api\/?$/, "")}/api`;
      }
      return `${origin}/api`;
    }
...
export const setCustomBackendIp = (ip: string) => {
  if (ip && ip.trim()) {
    ...
    API_BASE_URL = `http://${cleanIp}:8000/api`;
  } else {
